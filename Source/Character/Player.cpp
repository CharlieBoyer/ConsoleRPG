...
Player::Player(const std::string &name) :
	Character(name, 1, 30, 1, 0), _exp{0}, _nextLevelExp{GetNextLevelExp()} {}
...
void Player::LevelUp()
{
	...
	const int oldMaxHp = _maxHp;

	switch (choice)
	{
		case 1:
			_maxHp = static_cast<int>(std::round(_maxHp * 1.1f));
			_hp = _maxHp;
			std::cout << "Max HP increased: " << oldMaxHp << " \u2192 " << _maxHp << "\n" <<
				"    > HP fully restored.\n\n";
			break;
		case 2:
			_strength++;
